...
class TestValidateRut:
    ...
    def test_valid_ruts(self):
        ...
        for rut in valid_ruts:
            assert py_rut.validate_rut(rut), f"RUT {rut} should be valid"
    ...
    def test_invalid_ruts(self):
        ...
        for rut in invalid_ruts:
            assert not py_rut.validate_rut(rut), f"RUT {rut} should be invalid"
    ...
    def test_suspicious_ruts(self):
        ...
        for rut in suspicious_ruts:
            assert py_rut.validate_rut(rut, suspicious=False)
        
        # With suspicious flag, they should be invalid
        for rut in suspicious_ruts:
            assert not py_rut.validate_rut(rut, suspicious=True)
    
    def test_edge_cases(self):
        """Test edge cases"""
        # Minimum valid RUT
        assert py_rut.validate_rut("1-9")
        
        # RUT with spaces
        assert py_rut.validate_rut(" 12.345.678-5 ")
        
        # Mixed case K
        assert py_rut.validate_rut("7654321k") == py_rut.validate_rut("7654321K")
...
class TestFormatRut:
    ...
    def test_format_with_dots(self):
        ...
        for input_rut, expected in test_cases:
            result = py_rut.format_rut(input_rut, dots=True, uppercase=True)
    ...
    def test_format_without_dots(self):
        ...
        for input_rut, expected in test_cases:
            result = py_rut.format_rut(input_rut, dots=False, uppercase=True)
            assert result == expected, f"Format {input_rut} -> expected {expected}, got {result}"
    ...
    def test_format_lowercase(self):
        ...
        for input_rut, expected in test_cases:
            result = py_rut.format_rut(input_rut, dots=True, uppercase=False)
            assert result == expected, f"Format {input_rut} -> expected {expected}, got {result}"
    ...
    def test_format_combinations(self):
        ...
        assert py_rut.format_rut(input_rut, dots=True, uppercase=True) == "7.654.321-K"
        
        # dots=True, uppercase=False  
        assert py_rut.format_rut(input_rut, dots=True, uppercase=False) == "7.654.321-k"
        
        # dots=False, uppercase=True
        assert py_rut.format_rut(input_rut, dots=False, uppercase=True) == "7654321-K"
        
        # dots=False, uppercase=False
        assert py_rut.format_rut(input_rut, dots=False, uppercase=False) == "7654321-k"
    
    def test_format_ignore_invalid(self):
        """Test format with ignore_invalid flag"""
        # Invalid RUT should raise error by default
        with pytest.raises(Exception):
            py_rut.format_rut("invalid-rut", ignore_invalid=False)
...
class TestVerificationDigit:
    ...
    def test_verification_digit_string(self):
        ...
        for rut_body, expected_dv in test_cases:
            result = py_rut.verification_digit(rut_body)
    ...
    def test_verification_digit_edge_cases(self):
        ...
        large_num = 999999999
        dv = py_rut.verification_digit(large_num)
        assert isinstance(dv, str)
        assert dv in "0123456789K"
        
        # Test with zero
        assert py_rut.verification_digit(0) in "0123456789K"
    ...
    def test_verification_digit_invalid_input(self):
        ...
        for invalid_input in invalid_inputs:
            with pytest.raises((TypeError, ValueError)):
                py_rut.verification_digit(invalid_input)
...
class TestIntegration:
    ...
    def test_format_and_validate_cycle(self):
        ...
        for rut in test_ruts:
            # Format the RUT
            formatted = py_rut.format_rut(rut, dots=True, uppercase=True)
            # Validate the formatted RUT
            assert py_rut.validate_rut(formatted), f"Formatted RUT {formatted} should be valid"
    ...
    def test_verification_digit_integration(self):
        ...
        for body in test_bodies:
            dv = py_rut.verification_digit(body)
            full_rut = f"{body}-{dv}"
            assert py_rut.validate_rut(full_rut), f"RUT {full_rut} with calculated DV should be valid"
    ...
    def test_clean_and_validate(self):
        ...
        for dirty_rut in dirty_ruts:
            ...
            result = py_rut.validate_rut(dirty_rut)
...
class TestPerformance:
    ...
    def test_validate_many_ruts(self):
        ...
        # Validate all RUTs
        for rut in test_ruts:
            assert py_rut.validate_rut(rut), f"Generated RUT {rut} should be valid"
    
    def test_format_many_ruts(self):
        """Test formatting of many RUTs"""
        test_ruts = [f"1234567{i}-{py_rut.verification_digit(f'1234567{i}')}" 
                    for i in range(10)]
